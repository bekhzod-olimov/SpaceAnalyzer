...
# =================================================================================
UZBEK_TEXT = {
    "loading_model": "Yangi LLMDet modeli yuklanmoqda...",
...
def get_dominant_color(image, k=3):
    ...
    for color_name, (lower, upper) in UZBEK_TEXT["colors"].items():
        # XATO TO'G'RILANDI: `lower` o'rniga `lower[0]`
    ...
    # XATO TO'G'RILANDI: [10] va [11] o'rniga [1] va [2]
...
# =================================================================================
def main():
    model, processor, device = load_llmdet_model()
    ...
    while True:
        ...
        inputs = processor(images=image_pil, text=[CLASSES], return_tensors="pt").to(device)
        ...
        class_ids = [class_map.get(label, -1) for label in results["labels"]]
        ...
        all_detections = all_detections[all_detections.class_id != -1]
        ...
        cv2.imshow("Dynamic Store Analytics (LLMDet)", annotated_frame)
    ...
    if ret:
        # Heatmap uchun nuqtalarni to'plash mantiqini bu yerga qo'shish kerak
        # Hozircha bo'sh qoldirilgan, chunki u asosiy loopda edi
        pass
